...
| 7     | Recursion and formal verification of the lowering                                                                                                  | **done** (see `docs/README_phase7.md`; N formal verification of the lowering: N.1 spec, N.2 per-rule faithfulness, N.3 mutation harness; O recursion: O.1 a FRI-fold verifier in the language, O.2 a real fold verified inside an outer proof, and the full in-circuit FRI verifier; Merkle paths + fold + final check; verifying a real query inside an outer proof and in-circuit Fiat-Shamir deriving the fold challenge from the commitment and the query index made an algebraic reduction with the in-circuit index binding shown to need a decomposition hint, see docs/in-circuit-index.md. The `bits` decomposition hint added as a language primitive, a general range check the language could not express before, proved determinate without SMT by the closeBits rule, see docs/bits-hint.md) |
...
See `docs/README_phase7.md` for the full design note.
