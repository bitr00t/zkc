...
usage = mapM_ (hPutStrLn stderr)
  ...
  , "  zkc doc <file.zkc>   [-o <out.md>] [--field <name>]"
  , "  zkc lsp              run the language server (LSP over stdin/stdout)"
  , ""
  , "  doc generates the gadget reference from determinacy summaries — every"
  , "  line derived from what the compiler proved, never hand-written. It"
  , "  reports on the gadgets in scope, so point it at a file that uses them:"
